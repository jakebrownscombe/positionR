...
#'   (matches default additive integration where weights must sum to 1).
#' @param integration_method Character. Method for integrating detection and
#'   non-detection evidence. Options are:
#'   \itemize{
#'     \item "additive" (default): Original WADE formula, weighted sum of
#'       detection and inverted non-detection probabilities
#'       \code{(det * detection_weight) + ((1 - nondet) * non_detection_weight)}.
#'       Retains cells inside the detection buffer with graded probability; used
#'       in combination with \code{min_detection_prob} to suppress far-from-
#'       receiver cells that would otherwise pick up positive probability from
#'       the non-detection term alone.
#'     \item "subtractive": Detection field is the base; non-detection evidence
#'       subtracts from it: \code{det - (nondet * non_detection_weight)},
#'       clamped to 0. Produces tighter, detection-anchored position estimates
#'       but can over-penalise when non-detection evidence is strong.
...
#' @param scale_non_detections Logical. Whether to scale non-detection evidence by total
#'   detections per fish/time period. Default is TRUE. When TRUE, non-detection fields are
#'   proportionally stronger in periods with many detections (more missed transmissions
#'   expected), balancing the detection-weighted mean that favours high-count stations.
...
#' @param min_detection_prob Numeric. Minimum \code{weighted_mean_DE_normalized}
#'   value a cell must exceed to be retained in \code{position_probabilities}.
#'   Default is 0.05. This suppresses cells far from any receiver that would
#'   otherwise carry positive \code{integrated_prob} via the additive
#'   \code{(1 - nondet) * non_detection_weight} term alone, preventing inflated
#'   point clouds and UDs downstream. Set to 0 to disable.
...
calculate_fish_positions <- function(station_detections,
  ...
  position_probs <- aggregate_probability(
    ...
    scale_non_detections = scale_non_detections,
    min_detection_prob = min_detection_prob
  )
...
aggregate_probability <- function(df, detection_weight = 0.5, non_detection_weight = 0.5,
                                  integration_method = "additive", normalize_method = "global",
                                  scale_non_detections = TRUE,
                                  min_detection_prob = 0.05) {
  ...
  # Optionally scale non-detections by total detections per fish/time period
  if (scale_non_detections && nrow(det) > 0 && nrow(nondet) > 0) {
    total_dets <- det[, .(total_period_dets = sum(detections, na.rm = TRUE)),
                       by = .(fish_id, time_period)]
    max_dets <- max(total_dets$total_period_dets, na.rm = TRUE)
    if (max_dets > 0) {
      total_dets[, det_scale := total_period_dets / max_dets]
    } else {
      total_dets[, det_scale := 1]
    }
    nondet <- total_dets[nondet, on = .(fish_id, time_period)]
    nondet[, non_det_DE_normalized := non_det_DE_normalized * det_scale]
    nondet[, c("total_period_dets", "det_scale") := NULL]
  }
  ...
  # Drop cells with negligible detection evidence so far-from-receiver cells
  # (which pick up positive integrated_prob via the additive (1 - nondet) term
  # alone) don't pollute downstream sampling and plotting.
  if (!is.null(min_detection_prob) && min_detection_prob > 0) {
    combined <- combined[is.na(weighted_mean_DE_normalized) |
                         weighted_mean_DE_normalized > min_detection_prob]
  }
