...
- generating correlated random walks to simulate animal movements, and generate detections based on the range model 
...
ggplot2::ggplot(station_distances_df %>% dplyr::filter(station_no==10), 
...
```{r positioning, fig.width=7, fig.height=5}

# calculate detection fields for detected receivers and non-detected receivers 
positioning_results <- calculate_fish_positions(
  station_detections = fish_simulation$station_detections, # fish detections
  station_distances_df = station_distances_df,    # distances from each station to each raster cell
  receiver_stations = points_regular,             # receiver stations
  de_model = logistic_DE$log_model,               # Apply DE model
  bin_size_seconds = 3600,                        # 1 hour
  detection_weight = 0.5,                         # default 0.5 (equal weighting)
  non_detection_weight = 0.5,                     # default 0.5 (equal weighting)
  max_non_detection_distance = 2000,              # including non-detecting receivers 2km from detection
  normalization_method = "min_max",               # default, other options available 
  fish_id_col = "path_id",                        # Specify fish ID column
  time_col = "time_seconds",                      # Specify time column
  station_col = "station_id",                     # Specify station column
  verbose = TRUE                                  # Full verbose output
)


# Plot each panel separately (for R markdown output, can do type='all')
fish_select <- 1
time_select <- 0

plot_fish_positions(
  positioning_results = positioning_results,
  depth_raster_df = depth_raster,                # Optional depth data
  fish_select = fish_select,                     # Fish ID to plot
  time_select = time_select,                     # Time bin to plot
  prob_threshold = 0.05,                          # Minimum probability to display
  plot_type = "detection",                       # 'detection', 'non_detection', 'integrated', or 'all'
  return_list = FALSE                            # Return combined plot
)
...
plot_fish_positions(
  ...
  fish_select = fish_select,                     # Fish ID to plot
  time_select = time_select,                     # Time bin to plot
  plot_type = "non_detection",                   # 'detection', 'non_detection', 'integrated', or 'all'
  return_list = FALSE                            # Return combined plot
)
...
plot_fish_positions(
  ...
  time_select = time_select,                     # Time bin to plot
  prob_threshold = 0.05,                         # Minimum probability to display
  detection_threshold = 0.05,                    # added to limit integrated probabilities to within higher probability detection areas
  plot_type = "integrated",                      # 'detection', 'non_detection', 'integrated', or 'all'
  return_list = FALSE                            # Return combined plot
)
...
```{r particle filtering}

movement_params <- list(
  step_length_mean = 50,     # Same default as simulate_fish_tracks
  step_length_sd = 20,       # Same default
  turning_angle_mean = 0,    # Same default (degrees)
  turning_angle_sd = 45      # Same default (degrees)
)


# Extract parameters from your simulation
sim_params <- fish_simulation$parameters

# Use directly in particle filter
pf_results <- particle_filter_positioning(
  detection_data = fish_simulation$station_detections,
  station_coords = points_regular,
  de_model = logistic_DE$log_model,
  boundary_raster = depth_raster,
  n_cores = 12,
  movement_params = list(
    step_length_mean = sim_params$step_length_mean,      # 50
    step_length_sd = sim_params$step_length_sd,          # 20  
    turning_angle_mean = sim_params$turning_angle_mean,  # 0
    turning_angle_sd = sim_params$turning_angle_sd       # 45
  ),
  n_particles = 10
)
...
```{r particle filtering plot, fig.width=7, fig.height=5}
...
plot_particle_filter_results(
  ...
  actual_track = fish_simulation$fish_tracks  
)
